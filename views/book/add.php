...
<form method="post" action="<?= BASE_PATH.'book/add' ?>" enctype="multipart/form-data">
    <fieldset>
       ...
       <div class="form-group">-->
           <label for="exampleSelect1" name="id_genre" class="form-label mt-4">Catégorie</label>
           <select class="form-select" id="exampleSelect1">
               <?php foreach ($genres as $genre): ?>

             <option value="<?= $genre['id_categorie']?>">

             <?php if (isset($book) && $book['id_genre'] == $genre['id_categorie']): echo 'selected';  endif; ?>
                ><?= $genre['name']  ?>
            </option> 
